...
from telegallus.database.schemes import UserBot, AccountTgData, TelegramBot
...
async def handle_delete_data(callback_query: CallbackQuery, state: FSMContext):
    ...
    await callback_query.message.bot.edit_message_text(chat_id=callback_query.message.chat.id, message_id=callback_query.message.message_id,
        text="Выбери опцию", reply_markup=keyboard_management_bot(bots, account_id,user_id[0].user_bot_id)
    )
...
async def handle_delete_data(callback_query: CallbackQuery, state: FSMContext):
    await callback_query.message.bot.edit_message_text(chat_id=callback_query.message.chat.id, message_id=callback_query.message.message_id,
        text="Отправь token бота", reply_markup=keyboards_cancellation()
    )
    print(callback_query.data.split("."))
...
async def handle_delete_user(message: Message, state: FSMContext):
    ...
    bots = await TelegramBot(account_tg_pars_id=account_id).get_account_tg_data()
    try:
        client = await TelegramClient(StringSession(), api_id=accounts[0].api_id_account, api_hash=accounts[0].api_hash_account).start(bot_token=message.text.strip())
        session_str = client.session.save()
        tg_bot = await TelegramBot(bot_key_protected=message.text.strip(),session_id_bot=session_str, account_tg_pars_id=account_id).add_bots()
        print(tg_bot)
    ...
    await message.bot.edit_message_text(chat_id=message.chat.id, message_id=data.get("update_message"),
        text="Выбери пункт", reply_markup=keyboard_management_bot(bots, account_id,accounts[0].user_bot_id)
    )
