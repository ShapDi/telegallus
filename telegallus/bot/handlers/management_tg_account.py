...
from telegallus.bot.keyboards.main_menu_kb import (
    ...
    keyboard_management_tg_account,
)
...
async def handle_delete_data(callback_query: CallbackQuery, state: FSMContext):
    ...
    await callback_query.message.bot.edit_message_text(
        ...
        text="Выбери опцию",
        reply_markup=keyboard_management_tg_account(account),
    )
...
async def handle_delete_start(callback_query: CallbackQuery, state: FSMContext):
    await callback_query.message.bot.edit_message_text(
        chat_id=callback_query.message.chat.id,
        message_id=callback_query.message.message_id,
        text="Отправьте имя аккаунта для отображения(Оно должно быть уникально)",
...
async def handle_delete_user(message: Message, state: FSMContext):
    ...
    await message.bot.edit_message_text(
        ...
        text="Отправьте api_id для управления ботом",
        reply_markup=keyboards_cancellation(),
    )
...
async def process_delete_user(message: Message, state: FSMContext):
    ...
    await message.bot.edit_message_text(
        chat_id=message.chat.id,
        message_id=data.get("update_message"),
        text="Отправьте api_hash для управления ботом",
        reply_markup=keyboards_cancellation(),
    )
...
async def process_delete_user(message: Message, state: FSMContext):
    ...
    print(user_bot_id)
    req = await AccountTgData(
        name_account=data.get("name_account_tg"),
        api_id_account=str(data.get("api_id")),
        api_hash_account=str(data.get("api_hash")),
    ...
    await message.bot.edit_message_text(
        chat_id=message.chat.id,
        message_id=data.get("update_message"),
        text="Новый аккаунт добавлен",
        reply_markup=keyboard_management_tg_account(accounts),
    )
    await state.clear()
