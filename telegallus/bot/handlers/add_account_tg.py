...
from telegallus.bot.keyboards.main_menu_kb import keyboard_main, keyboards_cancellation
...
async def process_delete_user(message: Message, state: FSMContext):
    ...
    print(user_bot_id)
    req = await AccountTgData(name_account = data.get("name_account_tg"), api_id = str(data.get("api_id")), api_hash= str(data.get("api_hash")), user_bot_id=user_bot_id[0].id).add_account_tg_data()
    print(data)
    print(req)
    await message.bot.edit_message_text(chat_id=message.chat.id, message_id=data.get("update_message"),
        text="Новый аккаунт добавлен", reply_markup=keyboard_main()
    )
    await state.clear()