...
class AccountTgData(Base):
    ...
    async def get_tg_account(self):
        async with get_session_factory() as session:
            query = select(AccountTgData).where(
                or_(AccountTgData.user_bot_id == self.user_bot_id)
            )
            data = await session.execute(query)
            data = data.scalars().all()
            return data
    ...
    async def get_account_tg_data(self, session):
        query = select(AccountTgData).where(
            or_(AccountTgData.name_account == self.name_account)
        )
        data = await session.execute(query)
        data = data.scalars().all()
        return data
...
class TelegramBot(Base):
    ...
    async def get_telegram_bot(self, session):
        query = select(TelegramBot).where(
            or_(TelegramBot.account_tg_pars_id == self.account_tg_pars_id)
        )
        data = await session.execute(query)
        data = data.scalars().all()
        return data
...
class MonitoredChat(Base):
    ...
    async def add_monitored_chat(self):
        async with get_session_factory() as session:
            await self.create_record(session)
